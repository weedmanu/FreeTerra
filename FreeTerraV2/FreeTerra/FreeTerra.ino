 ...
 * gestion du brumisateur au point chaud
 * point froid
 */
...
#define pin_Relais_Brumisateur 16										  // D0 pin du Relais pour le Brumisateur
RegulHumi Brumisateur(pin_Relais_Brumisateur);        // On crée l'objet Brumisateur
...
int8_t TargetTemp, TargetTempJ, TargetTempN, TargetHumi, HysHumi, TempMin, TempMax;
...
void terrarium () {   
  if ((millis() - lastmillis) > INTERVALLE) {        
    ...
    if (Flag_Init == false) {        
      ...
      Serial.print("TargetHumi : ");            Serial.println(TargetHumi);
      Serial.print("HysHumi : ");               Serial.println(HysHumi);        
      ...
      Serial.print(HumiF);
      Serial.println(" %");   
      if (Chauffage.gestion(TempC, TargetTemp, HysTemp)) { Serial.println("chauffage ON"); }
      else { Serial.println("chauffage OFF"); }  
      if (Brumisateur.gestion(HumiC, TargetHumi, HysHumi)) { Serial.println("brumisateur ON"); }
      else { Serial.println("brumisateur OFF"); }
      if (Flotteur.verif()) { 
        Serial.println("Niveau OK"); 
        Flag_Alarme_Niveau_Eau = false;
      } else { 
        Buzzer.Alerte_Sonore(); 
        Serial.println("Niveau KO"); 
        Flag_Alarme_Niveau_Eau = true;
      }
...
void load_from_file(String file_name) {  
  ...
  if (SPIFFS.exists(file_name)) { 
    ...
    TargetHumi = (result.substring(i3 + 1, i4)).toInt();
    HysHumi = (result.substring(i4 + 1, i5)).toInt();
    TempMax = (result.substring(i5 + 1, i6)).toInt();
    TempMin = (result.substring(i6 + 1, i7)).toInt();
    Lever_H = (result.substring(i7 + 1, i8)).toInt();
    Lever_M = (result.substring(i8 + 1, i9)).toInt();
    Coucher_H = (result.substring(i9 + 1, i10)).toInt();
    Coucher_M = (result.substring(i10 + 1, i11)).toInt();
...
void recup_datas_appli() {      
  ...
  TargetHumi =(serveur.arg(3)).toInt();
  HysHumi = (serveur.arg(4)).toInt();
  TempMax =(serveur.arg(5)).toInt();
  TempMin = (serveur.arg(6)).toInt();
  Lever_H = (serveur.arg(7)).toInt();  
  Lever_M = (serveur.arg(8)).toInt();  
  Coucher_H = (serveur.arg(9)).toInt(); 
  Coucher_M = (serveur.arg(10)).toInt();   
  ...
  if(!file){    
    ...
    String Config = String(TargetTempJ) + "," + String(TargetTempN) + "," + String(HysTemp) + "," + 
                    String(TargetHumi) + "," + String(HysHumi) + "," + String(TempMax) + "," + String(TempMin) + "," + 
...
void envoyer_datas() {    
  String datas = String(date) + "," + String(heure) + "," + String(TargetTemp) + "," +  String(TempC) + "," + String(HumiC) + "," +  
                 ...
                 String(Flag_Alarme_Temp_Min) + "," + String(Flag_Alarme_Temp_Max) + "," + String(Flag_Alarme_Niveau_Eau);  
...
void setup() {
  ...
  serveur.on("/",handleRoot);                                   // on associe la fonction handleRoot a la racine du serveur
