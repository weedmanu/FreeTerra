...
    tempF = data[5];
	...
	if ( humiC < TargetHumi+5 && humiC > TargetHumi-5 ) { document.getElementById('humiC').style.color = '#C1FF6B';	} 
	else if ( humiC > TargetHumi+10 || humiC < TargetHumi-10 ) { document.getElementById('humiC').style.color = '#FFFF00'; }
	else if ( humiC > TempMax || humiC < TempMin ) { document.getElementById('humiC').style.color = '#FF7F7F'; }		
	else { document.getElementById('humiC').style.color = "#FF7F7F"; }
	...
	if ( humiF < TargetHumi+20 && humiF > TargetHumi-20 ) { document.getElementById('humiF').style.color = "#C1FF6B"; }
	else if ( humiF > TargetHumi+20 || humiF < TargetHumi-20 ) { document.getElementById('humiF').style.color = "#FF7F7F"; }
	else { document.getElementById('humiF').style.color = "#FF7F7F"; }
	
	if (AlarmeEau) { 
		img_AlarmeEau = '<img src="\soif.png" style="width:42px;height:42px;">';
		txt_AlarmeEau = "<span class='clignote'>Il n'y a plus d'eau</span>"; 
		document.getElementById('img_AlarmeEau').innerHTML = img_AlarmeEau;
		document.getElementById('txt_AlarmeEau').innerHTML = txt_AlarmeEau;
		document.getElementById('txt_AlarmeEau').style.color = '#FF7F7F';
	} else { 
		img_AlarmeEau = '<img src="\ok.png" style="width:42px;height:42px;">';
		txt_AlarmeEau = "Niveau d'eau correct"; 
		document.getElementById('img_AlarmeEau').innerHTML = img_AlarmeEau;
		document.getElementById('txt_AlarmeEau').innerHTML = txt_AlarmeEau;
		document.getElementById('txt_AlarmeEau').style.color = '#C1FF6B';
	}
	...
	if (AlarmeSondePC) { 
		...
		txt_AlarmeSondePC = "Lecture sonde OK"; 
		document.getElementById('img_AlarmeSondePC').innerHTML = img_AlarmeSondePC;
		document.getElementById('txt_AlarmeSondePC').innerHTML = txt_AlarmeSondePC;
	...
	if (AlarmeSondePF) { 
		...
		txt_AlarmeSondePF = "Lecture sonde OK"; 
		document.getElementById('img_AlarmeSondePF').innerHTML = img_AlarmeSondePF;
		document.getElementById('txt_AlarmeSondePF').innerHTML = txt_AlarmeSondePF;
...
function RecupConfig() {        
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			document.getElementById('TargetHumi').innerHTML = TargetHumi + ' %'; 
			document.getElementById('TargetHumi').style.color = '#1E90FF';			
			
			document.getElementById('HysHumi').innerHTML = HysHumi + ' %';		
			document.getElementById('HysHumi').style.color = '#1E90FF';			
			
			if   ( Lever_M < 10) { Lever = Lever_H + "h0" + Lever_M; }
			else { Lever =  Lever_H + "h" + Lever_M; }
			if   ( Coucher_M < 10) { Coucher = Coucher_H + "H0" + Coucher_M; }
			else { Coucher = Coucher_H + "H" + Coucher_M; }
